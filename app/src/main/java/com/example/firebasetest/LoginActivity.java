...
public class LoginActivity extends AppCompatActivity implements GoogleApiClient.OnConnectionFailedListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btn_google.setOnClickListener(new View.OnClickListener() { //구글 로그인 버튼 클릭시 이 부분 수행
            @Override
            public void onClick(View v) {
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) { // 구글 로그인 인증을 요청 했을 때 결과 값을 되돌려 받는 곳
        ...
        if(requestCode == REQ_SIGN_GOOGLE){
            ...
            if(result.isSuccess()){ // 인증결과가 성공적인가
                GoogleSignInAccount account = result.getSignInAccount(); // account 라는 데이터는 구글 로그인 정보를 담고 있다.(닉네임, 프로필, 이메일 주소 etc..)
